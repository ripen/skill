...
function menu_linkage($linkageid = 0, $id = 'linkid', $defaultvalue = 0) {
	...
	if($datas['style']=='1') {
		...
		$var_div = $defaultvalue && (ROUTE_A=='edit' || ROUTE_A=='account_manage_info'  || ROUTE_A=='info_publish') ? menu_linkage_level($defaultvalue,$linkageid,$infos) : $datas['title'];
 ...
 */
function string2img($txt,$fonttype = 5 ) {
	if(empty($txt)) return false;
	if(function_exists("imagepng")) {
		$txt = urlencode(sys_auth($txt));
		$txt = '<img src="'.APP_PATH.'api.php?op=creatimg&txt='.$txt.'" align="absmiddle">';
	}
	return $txt;
}
